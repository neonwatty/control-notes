...
class Setup:
    ...
    def choose_cost(self,name,**kwargs):
        ...
        self.cost_object = cost_functions.Setup(name,**kwargs)

        # if the cost function is a two-class classifier, build a counter too
        if name == 'softmax' or name == 'perceptron':
            self.count_object = cost_functions.Setup('twoclass_counter',**kwargs)
                        
        if name == 'multiclass_softmax' or name == 'multiclass_perceptron':
            self.count_object = cost_functions.Setup('multiclass_counter',**kwargs)
    ...
    def fit(self,**kwargs):
        ...
        weight_history,train_cost_history,val_cost_history = optimizers.gradient_descent(self.cost,self.w_init,self.x_train,self.y_train,self.x_val,self.y_val,self.alpha_choice,self.max_its,self.batch_size,verbose=verbose)
