...
class Setup:
    ...
    def preprocessing_steps(self,**kwargs):        
        ...
        self.x_normalizer = s.normalizer
        self.x_inverse_normalizer = s.inverse_normalizer
        
        s = normalizers.Setup(self.y,normalizer_name)
        self.y_normalizer = s.normalizer
        self.y_inverse_normalizer = s.inverse_normalizer
        
        # normalize input 
        self.x = self.x_normalizer(self.x)
        
        # normalize output
        self.y = self.y_normalizer(self.y)
    ...
    def show_histories(self,**kwargs):
        ...
        if self.train_portion == 1:
            ...
            self.val_accuracy_histories = [[] for s in range(len(self.val_cost_histories))]
